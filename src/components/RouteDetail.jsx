...
export default function RouteDetail({ route, formData, adminNotes, preferences = [], itemComments = {}, onSaveItemComment }) {
  ...
  const handleExport = async () => {
    setExporting(true);
    try {
      await exportRoutePDF(route, formData, adminNotes, itemComments);
    } catch (err) {
    ...
    <div className="border-t border-border">
      ...
      <div className="p-5 border-t border-border">
        <button
          ...
          disabled={exporting}
          className="w-full bg-accent hover:bg-accent/90 text-white font-semibold py-3 rounded-lg transition-colors cursor-pointer disabled:opacity-70 disabled:cursor-wait flex items-center justify-center gap-2"
        >
          {exporting ? (
            <>
